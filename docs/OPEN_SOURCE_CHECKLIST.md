...
4. **Binary evidence**: before publishing an artifact, run the fail-closed public-binary audit with a CycloneDX SBOM and hashed evidence for platform signing, macOS notarization where applicable, malware scanning, native inventory, and exact bundled-FFmpeg corresponding source. Windows one-file candidates are not public packages.
...
4. **二进制证据**：公开产物前必须运行失败关闭的 public-binary 审计，并提供
   CycloneDX SBOM，以及平台签名、适用时的 macOS 公证、恶意软件扫描、原生库清单
   和内置 FFmpeg 精确对应源码的哈希证据。Windows one-file 候选不能作为公开包。
