...
- `setuptools >= 77.0.3`: `dev` extra and PEP 517 build backend. This is the
  minimum that supports the project's PEP 639 SPDX `license` and `license-files`
  metadata; locking it lets CI install the editable project with
  `--no-build-isolation` instead of fetching an unreviewed build-tool version.
...
- `setuptools >= 77.0.3` 属于 `dev` extra 和 PEP 517 构建后端；这是支持本项目
  PEP 639 SPDX `license` 与 `license-files` 元数据的最低版本。将其锁定后，CI 可用
  `--no-build-isolation` 安装项目，避免临时下载未经锁定的构建工具版本。
