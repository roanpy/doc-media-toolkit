...
- 顶栏 `设置` 可配置 OpenAI 兼容接口。Base URL 只接受无内嵌凭据、查询或片段的 HTTP(S) 地址，远程地址必须使用 HTTPS（明文 HTTP 仅限本机 localhost），响应体有大小上限；API Key 仅保存在当前进程。未启用视觉能力时 AI 只读取代码候选与规格，启用后才发送压缩预览。AI 仅建议命名、分类、疑似合并组和主资源，不自动归并或删除。
...
图片和视频的新入库资源共用跨平台分类路径校验和保守名称整理：明确的 Unix 时间戳前缀移到名称末尾，中文名称末尾的 `fault` 规范为“异常”，原文件名和来源路径仍保留在清单中。程序不会根据文件夹或 AI 结果静默决定分类；手工填写的分类优先。图片按 SHA-256 与像素内容去重，视频按 SHA-256 与内容指纹匹配，名称不参与身份判断。AI 整理只接收代码规则筛出的候选及经确认的低清预览，提供名称、分类、标签和归并建议；应用任何建议前均需人工确认。

跨平台名称整理还会避开 Windows 保留设备名，并为长名称限制长度、给视频规格与哈希后缀预留空间；不会批量改写已有库。库根目录仍建议放在较短的路径下，完整路径长度也受操作系统和外部播放器限制。

图片重复入库前会检查库内实体及 SHA-256；文件已丢失或内容已变化时报告导入失败，不计作复用成功。此时先运行图片库体检，并从已知备份恢复或核对异常文件，再重试导入。

关闭或取消 AI 建议窗口会结束本次审阅，不再弹出后续合并窗口。接受建议后才进入逐组合并核对；只有合并建议而没有名称等字段时，也有明确的继续审阅入口。文件合并仍需单独确认。
