...
class MetadataDialog(QDialog):
    def __init__(self, parent: QWidget, asset: dict) -> None:
        ...
        self.tags_input = QLineEdit("，".join(asset.get("tags") or []))
        ...
        form.addRow("说明", self.summary_input)
    ...
    def values(self) -> dict:
        tags = self.tags_input.text().replace(",", "，").split("，")
        return {
...
class ImportPreviewDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        layout = QVBoxLayout(self)
        hint = QLabel(
            "以下为将从图片或文档中提取的实际图片。这里只预览，不修改源文件。"
        )
        ...
        self.list.setHeaderLabels(["来源文件", "文档内位置", "规格"])
...
class SimilarImageReviewDialog(QDialog):
    def __init__(
        ...
        super().__init__(parent)
        self.decision = ""
        self.setWindowTitle("核对相似图片")
        self.resize(760, 520)
        self.setMinimumSize(680, 460)
        self.setStyleSheet(IMAGE_MANAGER_STYLESHEET)
        layout = QVBoxLayout(self)
        hint = QLabel(
            f"代码相似分 {score:.1f}。请查看两张图片后决定；"
            "合并只整理图片库，原始图片和文档不受影响。"
        )
        hint.setObjectName("dialogSummary")
        hint.setWordWrap(True)
        layout.addWidget(hint)
        previews = QHBoxLayout()
        for label, asset in (("左图", left), ("右图", right)):
            pane = QVBoxLayout()
            title = QLabel(
                f"{label} · {asset['name']}\n"
                f"{asset['width']}×{asset['height']} · {asset['format']} · "
        ...
        buttons = QHBoxLayout()
        ignore = QPushButton("不是同一张")
        ignore.setToolTip("记住这对图片，以后不再作为相似候选提示。")
        ignore.clicked.connect(lambda: self._choose("ignore"))
        close = QPushButton("关闭")
        close.clicked.connect(self.reject)
        keep_left = QPushButton("保留左图并合并")
        keep_left.clicked.connect(lambda: self._choose("left"))
        keep_right = QPushButton("保留右图并合并")
...
class PendingImageCleanupDialog(QDialog):
    def __init__(self, parent: QWidget, window: MainWindow) -> None:
        ...
        self.tree.setHeaderLabels(["文件", "原位置", "大小", "原因", "隔离时间"])
        ...
        self.empty_button = QPushButton("永久清空")
    ...
    def reload(self) -> None:
        ...
        self.summary.setText(
            (
                f"共 {len(entries)} 个文件，{format_user_file_size(total)}。"
    ...
    def restore_selected(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "图片待清理目录", str(exc))
            return
        self.window.status_label.setText(f"状态与日志 · 已还原 {restored.name}")
    ...
    def empty_all(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "图片待清理目录", str(exc))
            return
        self.window.status_label.setText(f"状态与日志 · 已永久清空 {removed} 个文件")
        self.window.refresh_views()
        self.reload()
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.setWindowTitle("图片库 - 文档媒体工具箱")
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("文档图片资产库")
        title.setObjectName("pageTitle")
        subtitle = QLabel(
            "管理独立图片及 PPTX / DOCX / 数字版 PDF 内实际引用的图片，不做图片回填"
        )
        ...
        self.import_drop_label = QLabel("可拖入或多选图片、PPTX、DOCX、数字版 PDF")
        ...
        self.choose_import_button = QPushButton("选择图片 / 文档（可多选）")
        ...
        for label, button in (
            ("清空待入库", self.clear_import_button),
            ("预览提取内容", self.preview_import_button),
            ("开始入库", self.import_button),
        ):
        ...
        self.library_filter_input.setPlaceholderText("筛选名称、分类、格式或来源")
        ...
        filter_row.addWidget(QLabel("查找"))
        ...
        for key, title, tooltip in (
            ("all", "全部", "显示图片库全部资产"),
            ("duplicate_origins", "重复来源", "同一图片被多个文件或文档引用"),
            ("similar", "相似", "存在尚未确认的代码相似候选"),
            ("undersized", "过小", "低于当前入库最小尺寸设置"),
            ("no_origin", "无来源", "图片资产没有任何来源记录"),
        ):
            button = QPushButton(f"{title} 0")
            button.setObjectName("statFilter")
            button.setCheckable(True)
            button.setFixedHeight(32)
            button.setToolTip(f"{tooltip}；点击只筛选，不修改图片库。")
        ...
        self.detail_title = QLabel("选择图片查看详情")
        ...
        self.detail_origins.setHeaderLabels(["来源", "文档内位置"])
        ...
        self.status_label = QLabel("状态与日志 · 等待开始")
    ...
    def on_activated(self) -> None:
        ...
        try:
            ...
            self.status_label.setText(f"状态与日志 · 重新载入失败：{exc}")
            return
        self.refresh_views()
    ...
    def choose_create_project(self) -> None:
        directory = QFileDialog.getExistingDirectory(self, "新建图片库")
        if directory:
            try:
                project = ImageProject.create(Path(directory), "图片库")
            except Exception as exc:
    ...
    def choose_open_project(self) -> None:
        directory = QFileDialog.getExistingDirectory(self, "打开图片库")
        if directory:
    ...
    def _set_project(self, project: ImageProject) -> None:
        ...
        self.project_path.setText(
            f"当前图片库：{project.data['name']} · {project.root}"
        )
        self.status_label.setText(
            "状态与日志 · 图片库已打开"
            + ("，已从最近有效备份恢复" if project.recovered_from_backup else "")
        )
        if project.recovered_from_backup:
            QMessageBox.warning(
                self,
                "图片库已恢复",
                "当前清单不可读，已从最近有效备份恢复。建议立即执行“库体检”。",
            )
        self.refresh_views()
    ...
    def choose_import(self) -> None:
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "",
            (
                "支持的文件 (*.png *.jpg *.jpeg *.webp *.gif *.bmp *.tif *.tiff "
                "*.pptx *.docx *.pdf);;所有文件 (*)"
            ),
        )
        if files:
    ...
    def _refresh_pending_import(self) -> None:
        if self.pending_paths:
            names = "、".join(path.name for path in self.pending_paths[:3])
            more = (
                f" 等 {len(self.pending_paths)} 个文件"
                if len(self.pending_paths) > 3
                else ""
            )
            self.import_drop_label.setText(f"待入库：{names}{more}")
        else:
            self.import_drop_label.setText("可拖入或多选图片、PPTX、DOCX、数字版 PDF")
        self._sync_actions()
    ...
    def import_paths(self, paths: list[Path]) -> None:
        if self.project is None or self.worker_thread is not None:
            self.show_error("请先新建或打开图片库。")
            return
        project = self.project
        category = self.import_category_input.text().strip()
        min_width = self.min_width_spin.value()
        min_height = self.min_height_spin.value()
        self.status_label.setText("状态与日志 · 正在提取并整理图片…")
    ...
    def _import_succeeded(self, value: object) -> None:
        if not isinstance(value, dict):
            self._import_failed("图片入库返回了无效结果。")
            return
        result = value
        failed = len(result["failed"])
        skipped = len(result["skipped"])
        self.status_label.setText(
            f"状态与日志 · 新增 {result['added']}，复用 {result['reused']}，"
            f"排除 {skipped}，失败 {failed}"
            + ("，已取消后续文件" if result.get("cancelled") else "")
        )
        self.refresh_views()
        ...
        if failed:
            ...
            QMessageBox.warning(self, "部分图片未导入", details)

    def _import_failed(self, message: str) -> None:
        self.status_label.setText("状态与日志 · 图片入库失败")
    ...
    def refresh_views(self) -> None:
        ...
        if self.project is None:
            ...
            self.pending_cleanup_button.setText("待清理 0")
            self.pending_cleanup_action.setText("待清理 (0)")
        ...
        self.pending_cleanup_button.setText(f"待清理 {len(pending)}")
        self.pending_cleanup_action.setText(f"待清理 ({len(pending)})")
        self.pending_cleanup_button.setToolTip(
            f"当前待清理 {len(pending)} 个文件，"
            f"{format_user_file_size(pending_size)}；可还原或手动永久清空。"
        )
    ...
    def _sync_actions(self) -> None:
        ...
        self.ai_button.setToolTip(
            ...
            if ai_configured
            else "请先点击顶栏齿轮配置并验证 AI；未配置时图片库功能不受影响。"
        )
    ...
    def refresh_detail(self) -> None:
        ...
        if asset is None or self.project is None:
            self.detail_title.setText("选择图片查看详情")
            self.preview.setPixmap(QPixmap())
        ...
        self.detail_meta.setText(
            f"{asset['width']}×{asset['height']} · {asset['format']} · "
            f"{format_user_file_size(asset['size_bytes'])}\n"
            f"SHA-256：{asset['sha256'][:16]}… · 来源 {len(origins)} 处\n"
            f"分类：{asset.get('category') or '未分类'} · "
            f"标签：{'、'.join(asset.get('tags') or []) or '无'}"
        )
        self.detail_summary.setText(asset.get("summary") or "暂无说明。")
        self.detail_origins.clear()
        for origin in origins:
            source_path = str(origin.get("source_path") or "未知来源")
            source_item = QTreeWidgetItem(
                [
                    source_path,
                    str(origin.get("member_path") or "独立图片"),
                ]
            )
    ...
    def show_similar(self) -> None:
        ...
        while True:
            ...
            self.refresh_views()
        if reviewed:
            self.status_label.setText(f"状态与日志 · 已核对 {reviewed} 组相似图片")
            self.refresh_views()
            return
        if not self.project.similar_candidates(current_id):
            QMessageBox.information(
                self, "相似候选", "代码指纹没有找到足够接近的候选。"
            )
    ...
    def reset_ignored_similar(self) -> None:
        ...
        try:
            ...
            self.show_error(str(exc))
            return
        self.status_label.setText(f"状态与日志 · 已重置 {count} 组忽略的相似候选")
        self.refresh_views()
    ...
    def request_ai_suggestion(self) -> None:
        ...
        if self.ai_thread is not None:
            ...
            self.status_label.setText("状态与日志 · 已取消显示；网络请求会在后台结束")
            return
        ...
        if not settings.value(privacy_key, False, bool):
            sent_content = (
                ...
                else "名称、规格、大小与代码相似度（不发送图片）"
            )
            answer = QMessageBox.question(
                self,
                "AI 图片分析",
                (
                    "将向已配置的 AI 服务发送所选图片和最多 5 张代码相似候选的"
                    f"{sent_content}，不发送原文档或本地路径。是否继续？"
                ),
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No,
            )
        ...
        self.status_label.setText("状态与日志 · AI 正在分析图片与相似候选…")
        ...
        self.ai_worker = OperationWorker(
            lambda _message, cancelled: OpenAICompatibleClient(config).organize_media(
                items, media_kind="图片", cancel_callback=cancelled
            )
        )
    ...
    def _show_ai_suggestion(self, result: object) -> None:
        ...
        try:
            ...
            self.status_label.setText("状态与日志 · 对应图片已不存在，AI 结果已忽略")
            self._finish_ai()
            return
        ...
        if merged:
            applied.append(f"合并 {merged} 张重复图片")
        self.status_label.setText(
            "状态与日志 · AI 建议已完成"
            + (f"，已应用：{'、'.join(applied)}" if applied else "，未修改图片库")
        )
        ...
        for group in groups:
            ...
            answer = QMessageBox.question(
                ...
                (
                    ...
                    "确认后会合并来源、标签和说明，并删除库内其他副本；"
                    "原始文档和原始图片不受影响。是否确认它们内容相同？"
                ),
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No,
            )
    ...
    def _ai_failed(self, message: str) -> None:
        if self.ai_ignore_result:
            return
        self.status_label.setText("状态与日志 · AI 分析失败")
        self._finish_ai()
    ...
    def remove_selected(self) -> None:
        ...
        try:
            ...
            self.show_error(str(exc))
            return
        self.status_label.setText(f"状态与日志 · 已移除 {asset['name']}")
        self.refresh_views()
    ...
    def cleanup_orphans(self) -> None:
        ...
        if not orphans:
            self.status_label.setText("状态与日志 · 没有未引用文件")
            return
        answer = QMessageBox.question(
            self,
            "清理未引用文件",
            (
                f"发现 {len(orphans)} 个不在图片库清单中的文件。\n"
                "这些文件将移入待清理目录，可在“更多操作”中还原；"
                "原始文档不受影响。是否继续？"
            ),
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        ...
        self.status_label.setText(
            f"状态与日志 · 已将 {len(removed)} 个未引用文件移入待清理"
        )
    ...
    def show_library_health(self) -> None:
        ...
        details = [
            f"图片资产：{report['asset_count']}",
            f"待清理：{report['pending_cleanup_count']}",
            f"缺失文件：{len(report['missing_files'])}",
            f"内容大小变化：{len(report['modified_files'])}",
            f"未登记文件：{len(report['orphan_files'])}",
            f"待清理索引异常：{len(report['pending_cleanup_issues'])}",
        ]
        QMessageBox.information(
            self,
            "图片库体检",
            ("未发现异常。\n\n" if not issue_count else "发现需要处理的项目。\n\n")
            + "\n".join(details),
        )
        self.status_label.setText(
            "状态与日志 · 图片库体检完成，"
            + ("未发现异常" if not issue_count else f"发现 {issue_count} 项异常")
        )
    ...
    def show_error(self, message: str) -> None:
        QMessageBox.warning(self, "图片库", message)
